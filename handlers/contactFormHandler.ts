  ...
  return async ({ request, getClientAddress }: RequestEvent): Promise<Response> => {
    try {
      ...
      if (sanitizedData.recaptchaToken) {
        const isValidRecaptcha = await verifyRecaptchaToken(
      ...
      if (customSuccessHandler) {
        try {
          const customResult = await customSuccessHandler(
            sanitizedData,
            clientAddress,
          );
          if (customResult) {
...
export interface ContactFormHandlersOptions extends ContactApiHandlerOptions {
  // Inherits all options from ContactApiHandlerOptions
}
 ...
 * ```
 */
export function createContactFormHandlers(options: ContactFormHandlersOptions = {}): ContactFormHandlers {
  return {
