...
export interface EnhancedContactFormConfig extends ContactSchema {
  ...
  formDataParser: (formData: FormData, categorySlug: string) => Promise<{ data: FormData; errors: Record<string, string> }>;
...
export function validateContactConfig(config: Partial<ContactSchema>): ValidationError {
  ...
  usedFields.forEach((field) => {
    if (!config.fieldConfigs || !config.fieldConfigs[field]) {
      errors.push(
        `Field "${field}" is used in categories but has no configuration`,
      );
    }
  });
...
export function initContactFormConfig(
  ...
  return {
    ...
    createSubmissionHandler: async (context: any = {}): Promise<SubmissionHandler> => {
