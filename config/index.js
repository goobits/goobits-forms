...
export function initContactFormConfig(userConfig = {}) {
  ...
  currentConfig.formDataParser = async (formData, category) => {
    try {
      ...
      const schema =
        currentConfig.schemas.categories[category] ||
        currentConfig.schemas.categories.general;
      ...
      if (formData.getAll("attachments[]")?.length) {
        ...
        formDataObj.attachments = formData
          .getAll("attachments[]")
          .map((file) => ({
            file,
            name: file.name,
      ...
      return {
        data: {},
        errors: { _form: "An error occurred while processing the form data." },
      };
    }
  };
  ...
  currentConfig.createSubmissionHandler = async ({ locals }) => {
    ...
    const defaultRecipient =
      currentConfig.defaultRecipient || "contact@example.com";
    const defaultSubject =
      currentConfig.defaultSubject || "New Contact Form Submission";
    ...
    return async (data, category) => {
      try {
        ...
        const subject =
          categoryConfig.subject ||
          `${defaultSubject}: ${categoryConfig.label || category}`;
...
function buildValidationSchemas(config) {
    ...
    checkbox: (fieldName) =>
      z
        ...
        .refine((value) => value === "on" || value === true, {
          message:
            errorMessages[fieldName] || errorMessages.required(fieldName),
        }),
    select: () => z.string(),
  ...
  // Build field schemas
  const schemas = Object.entries(fieldConfigs).reduce(
    (acc, [fieldName, fieldConfig]) => {
      ...
      if (
        ...
        ["email", "tel", "text", "textarea", "url"].includes(type)
      ) {
        schema = schema.max(
          fieldConfig.maxlength,
          `Maximum ${fieldConfig.maxlength} characters`,
        );
      }
  ...
  schemas.attachments = z
    .array(
      z.object({
        file: z
          .instanceof(File)
          .refine(
            (file) => file.size <= fileSettings.maxFileSize,
            errorMessages.fileSize,
          )
          .refine(
            (file) => fileSettings.acceptedImageTypes.includes(file.type),
    ...
    .max(3, errorMessages.maxFiles)
    .optional();

  // Build category schemas
  const categorySchemas = Object.entries(config.categories).reduce(
