 ...
 */
export type RateLimiter = (locals: Record<string, unknown>) => Promise<{ allowed: boolean; retryAfter?: number }>;

/**
 * Data sanitizer function type
 */
export type DataSanitizer = (
...
export interface ContactFormConfig {
	...
	categories: Record<string, { fields: string[]; label?: string }>;
	/** Validation schemas */
	schemas: {
		categories: Record<string, AnyZodObject>;
	};
 ...
 * ```
 */
export function createContactPageHandler(
	options: ContactPageHandlerOptions = {}
): (event: { url: URL }) => Promise<ContactPageLoadResult> {
	const { getCategory = (url: URL) => url.searchParams.get('type') || 'general', config = null } =
		options;

	return async ({ url }: { url: URL }): Promise<ContactPageLoadResult> => {
 ...
 */
export interface ContactHandlersOptions
	extends ContactGetHandlerOptions,
		ContactPostHandlerOptions {
