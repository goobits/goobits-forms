 ...
 */
export type ValidatorFunction = (category: string) => unknown;
...
export type SubmissionHandlerCreator = (
	locals: App.Locals
) => Promise<(data: Record<string, unknown>, category: string) => Promise<void>>;
...
export type ErrorHandler = (
	error: Error,
	context: { data: Record<string, unknown>; slug: string }
...
export interface ContactRouteHandlers {
	...
	load: (params: RequestEvent) => Promise<LoadResult>;
...
export function createContactRouteHandlers(config: CategoryRouterConfig): ContactRouteHandlers {
	...
	return {
		load: (params: RequestEvent) => router.load(params),
		actions: {
