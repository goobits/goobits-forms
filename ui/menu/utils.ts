...
export function calculateMenuPosition(
	...
	finalX = Math.max(padding, Math.min(finalX, viewportWidth - menuDimensions.width - padding));
	finalY = Math.max(padding, Math.min(finalY, viewportHeight - menuDimensions.height - padding));
...
function getFocusableItems(items: MenuItem[]): MenuItem[] {
	return items.filter(
		(item) =>
			item.type !== 'separator' && item.type !== 'label' && !('disabled' in item && item.disabled)
	);
}
...
function getNextFocusableIndex(
	...
	if (direction === 'next') {
		...
		const prevIndex =
			currentFocusableIndex <= 0 ? focusableIndices.length - 1 : currentFocusableIndex - 1;
...
export function createKeyboardNavigation(
	...
	return {
		...
		focusFirst() {
			const firstFocusable = items.findIndex((item) => focusableItems.includes(item));
		...
		focusLast() {
			const focusableIndices = items
				.map((item, index) => (focusableItems.includes(item) ? index : -1))
				.filter((index) => index !== -1);
			focusedIndex = focusableIndices.length ? focusableIndices[focusableIndices.length - 1] : -1;
			return focusedIndex;
		},
