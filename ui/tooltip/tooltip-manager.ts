...
function calculatePosition(
  ...
  event?: Event
): { position: { x: number; y: number }; finalPosition: 'top' | 'bottom' | 'left' | 'right'; arrowPosition: { x: number; y: number }; arrowVisible: boolean } {
...
function showTooltipInternal(
  ...
  const content = processContent(
    tooltipOptions.content || tooltipOptions.text || '',
    event
  );
