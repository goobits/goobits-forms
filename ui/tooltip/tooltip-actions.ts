...
const FADE_IN_DELAY = 0; // Start fading in immediately
const FADE_OUT_DELAY = 150; // Wait before starting fade out
const TRANSITION_DURATION = 200; // CSS transition duration
const STAGGER_DELAY = 50; // Small delay for coordinated animations
 ...
 */
export function useTooltip(node: HTMLElement, options: Partial<TooltipOptions> = {}): ReturnType<TooltipAction> {
    ...
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          elements.forEach((el) => {
            if (!el.classList.contains('no-tooltip') && !el.hasAttribute('data-tooltip-initialized')) {
